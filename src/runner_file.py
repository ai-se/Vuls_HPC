...
#################
def parse_data(path='../../Datasets/firefox/', file1='crashes.csv',file2='vulns.csv',mode='source'):

    from ast_func import ast_func
    crashes = pd.read_csv('../data/' + str(file1))
    vulns = pd.read_csv('../data/' + str(file2))
    data = pd.merge(crashes, vulns, how='left', on=['file','crashes'])
    data['severity'].fillna(int(0), inplace=True)
    newcol = []
    ...
    data['sourcecode'] = newcol
    set_trace()
    data.to_csv('../data/vuls_data_ast.csv')
...
def divide_data(file='../../Datasets/vulns/vuls_data_new.csv'):
    import pandas as pd
    body = pd.read_csv(file)
    dom = body[body.apply(lambda x: x['file'].startswith('dom'), axis=1)]
    js = body[body.apply(lambda x: x['file'].startswith('js'), axis=1)]
    netwerk = body[body.apply(lambda x: x['file'].startswith('netwerk'), axis=1)]
    gfx = body[body.apply(lambda x: x['file'].startswith('gfx'), axis=1)]
    other = body[body.apply(lambda x: not (x['file'].startswith('dom') or x['file'].startswith('js') or x['file'].startswith('netwerk') or x['file'].startswith('gfx')), axis=1)]

    print(Counter(dom.severity>0))
    print(Counter(dom.crashes>0))
    print(dom.shape[0])
    print(Counter(js.severity>0))
    print(Counter(js.crashes>0))
    print(js.shape[0])
    print(Counter(netwerk.severity>0))
    print(Counter(netwerk.crashes>0))
    print(netwerk.shape[0])
    print(Counter(gfx.severity>0))
    print(Counter(gfx.crashes>0))
    print(gfx.shape[0])
    print(Counter(other.severity>0))
    print(Counter(other.crashes>0))
    print(other.shape[0])


    set_trace()
    dom.to_csv('../data/vuls_data_dom.csv')
    js.to_csv('../data/vuls_data_js.csv')
    netwerk.to_csv('../data/vuls_data_netwerk.csv')
    gfx.to_csv('../data/vuls_data_gfx.csv')
    other.to_csv('../data/vuls_data_other.csv')
...
def supervised(file):
    ...
    # xx = Counter([file.split('/')[0] for file in read.body['file'][poses]])
    # print(xx)
    # set_trace()

    # read.plot_lda()

    def split_data(body,seed):
        np.random.seed(seed)
        poses = np.where(np.array(body['label']) == "yes")[0]
        negs = np.where(np.array(body['label']) == "no")[0]

        train_pos = np.random.choice(poses, int(len(poses)*0.3))
        train_neg = np.random.choice(negs, int(len(negs)*0.3))
        train = list(train_pos)+list(train_neg)
        test = list(set(poses)-set(train_pos))+list(set(negs)-set(train_neg))
        return train,test
    ...
    pred = []
    # pred2 = []
    read.body['sourcecode'].fillna('', inplace=True)
    for i in xrange(repeats):
        train,test = split_data(read.body,i)
        poses = np.where(np.array(read.body['label']) == "yes")[0]
        negs = np.where(np.array(read.body['label']) == "no")[0]
        train_pos = list(set(train) & set(poses))
        train_neg = list(set(train) & set(negs))
        #
        # tfidfer = TfidfVectorizer(lowercase=False, stop_words=None, norm=None, use_idf=False, smooth_idf=False,
        #                         sublinear_tf=False,decode_error="ignore",max_features=1000)
        # tfidfer.fit(read.body['sourcecode'][train_pos])
        # pos_term = tfidfer.vocabulary_.keys()
        # tfidfer.fit(read.body['sourcecode'][train_neg])
        # neg_term = tfidfer.vocabulary_.keys()
        # # voc = (set(pos_term) | set(neg_term))-(set(pos_term) & set(neg_term))
        # voc = (set(pos_term) | set(neg_term))
        # # print(len(voc))
        # tfer = TfidfVectorizer(lowercase=False, stop_words=None, norm=None, use_idf=False,
        #                 vocabulary=voc,decode_error="ignore")
        # csr_mat=tfer.fit_transform(read.body['sourcecode'])
        #
        #
        # csr_mat = sparse.hstack((csr_mat,np.array(read.body['crashes'])[:,None]))


        # csr_mat = preprocessing.normalize(csr_mat,norm='l2',axis=1)

        # from sklearn.decomposition import PCA
        # norm = PCA(n_components=2)
        # csr_mat = norm.fit_transform(csr_mat.todense())


        csr_mat = read.csr_mat
        ...
        clf.fit(csr_mat[train], read.body['label'][train])


        ### agressive undersampling
        train_dist = clf.decision_function(csr_mat[train_neg])
        pos_at = list(clf.classes_).index("yes")
        if pos_at:
            train_dist=-train_dist
        negs_sel = np.argsort(train_dist)[::-1][:len(train_pos)]
        sample = list(train_pos) + list(np.array(train_neg)[negs_sel])
        clf.fit(csr_mat[sample], read.body['label'][sample])
        #####


        prediction = clf.predict(csr_mat[test])
        # prediction2 = [prediction[i] if int(read.body['crashes'][id])>0 else 'no' for i,id in enumerate(test)]
        pred.extend(prediction)
        # pred2.extend(prediction2)
        true.extend(read.body['label'][test])


    # set_trace()
    result1 = ABCD(true, pred)
    stats1 = result1()['yes'].stats()
    print(stats1)

    # result2 = ABCD(true, pred2)
    # stats2 = result2()['yes'].stats()
    # print(stats2)

def CRASH(filename, stop='true', error='none', interval = 100000, seed=0):
    stopat = 1
    thres = 0
    starting = 1
    counter = 0
    ...
    with open("../dump/"+filename+"_crash.pickle","wb") as handle:
        pickle.dump(result,handle)
    return read


def BM25(filename, stop='true', error='none', correct = 'no', interval = 100000, seed=0):
    stopat = 1
    thres = 0
    starting = 1
    counter = 0
...
def Metrics(filename, stop='true', error='none', interval = 100000, seed=0):
    stopat = 1
    thres = 0
    starting = 1
    counter = 0
    ...
    read.interval = interval
    if filename=='vuls_data_other.csv' or filename=='vuls_data_new.csv':
        read.step = 1000
    ...
    result['est'] = (read.record_est)
    result['pos'] = (read.record)
    with open("../dump/"+filename+"_metrics.pickle","wb") as handle:
        pickle.dump(result,handle)
    return read

def Random(filename, stop='true', error='none', error_rate = 0.5, correct = 'no', interval = 100000, seed=0, neg_len=0.5):
    stopat = 1
    thres = 0
    starting = 1
    counter = 0
...
def Rand(filename, stop='true', error='none', interval = 100000, seed=0):
    stopat = 1
    thres = 0
    starting = 10000
    counter = 0
    ...
    result['pos'] = (read.record)
    with open("../dump/"+filename+"_random.pickle","wb") as handle:
        pickle.dump(result,handle)
    return read
...
def error_hpcc_feature(fea, seed = 1):
    ...
    for file in files:
        print(str(seed)+": "+file+": "+ fea+ ": ", end='')
        if fea == 'combine':
            result = BM25(file,stop='mix',seed=seed)
        elif fea == 'text':
            result = Random(file,stop='mix',seed=seed)
        elif fea == 'metrics':
            result = Metrics(file,stop='true',seed=seed)
        elif fea == 'random':
            result = Rand(file,stop='true',seed=seed)
        else:
            result = Rand(file,stop='true',seed=seed)

        results[file] = {'pos':result.record,'est':result.record_est}
